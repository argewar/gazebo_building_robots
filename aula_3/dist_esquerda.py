...
class DistEsquerda(Node):
    def __init__(self):
        super().__init__('dist_esquerda')
        self.create_subscription(LaserScan, '/scan', self.cb, qos_profile_sensor_data)
        self.pub = self.create_publisher(Float32, '/dist_esquerda', 10)
        self.get_logger().info('Publicando /dist_esquerda (setor +90° ±30°)...')

    def cb(self, scan: LaserScan):
        centro = math.radians(+90.0)
